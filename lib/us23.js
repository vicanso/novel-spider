...
class US23 extends Novel {
  ...
  getIntroduction() {
    ...
    return this.search().then((id) => {
      ...
      data.desc = $('#content dd').eq(3).find('p').eq(1).text().trim();
  ...
  getChapter(start, end) {
    const getContent = (url) => {
      return utils.gbkGet(url).then((res) => {
    ...
    return this.getChapters().then((chapters) => {
      const max = Math.min(chapters.length, end);
      const urls = _.map(_.range(start, max), (index) => {
        return _.get(chapters, `[${index}].url`);
      });
      return Promise.map(urls, url => getContent(url), {
        concurrency: 10,
      });
    });
  }
  getAllInfos() {
    return this.getChapters().then((chapters) => {
      return this.getChapter(0, chapters.length);
    }).then((data) => {
      const chapters = _.map(this.options.chapters, (chapter, i) => {
        return {
          title: chapter.title,
          content: data[i],
          wordCount: data[i].length,
        };
      });
