...
const baseUrl = 'http://www.x23us.com';
...
class US23 extends Novel {
  ...
  getID(html) {
    ...
    _.forEach(trList, (tr) => {
      ...
      if (infos[0] === this.name && infos[2] === this.author) {
        ...
        if (href) {
          found = href.replace(`${baseUrl}/book/`, '');
        }
      }
    });
    return found;
  ...
  search() {
    ...
    const url = `${baseUrl}/modules/article/search.php?searchtype=articlename&searchkey=${key}`;
  ...
  getIntroduction() {
    ...
    return this.search().then((id) => {
      ...
      data.name = $('#content h1').first().text()
        .replace('全文阅读', '')
        .trim();
      const infoTdList = $('#content table tr').first().find('td');
      data.category = infoTdList.eq(0).text().trim();
      data.author = infoTdList.eq(1).text().trim();
      data.end = infoTdList.eq(2).text().trim() === '已完成';
      data.chapterUrl = $('.btnlinks a.read').first().attr('href');
      data.cover = `${baseUrl}${$('#content .fl .hst img').first().attr('src')}`;
      data.desc = $('#content dd').eq(3).find('p')
        .eq(1)
        .text()
        .trim();
  ...
  getChapter(index) {
    const getContent = url => utils.gbkGet(url).then((res) => {
      const reg = /[(２３Ｕｓ．ｃｏｍ)(x23us.com)(23us.com)(顶点小说网)]/gi;
      const $ = cheerio.load(res.body);
      const contentList = [];
