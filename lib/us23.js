...
class US23 extends Novel {
  ...
  getID(html) {
    ...
    _.forEach(trList, (tr) => {
      ...
      if (infos[0] === this.name && infos[2] === this.author) {
        ...
        if (href) {
          found = href.replace('http://x23us.com/book/', '');
        }
      }
    });
    return found;
  ...
  search() {
    ...
    const url = `http://x23us.com/modules/article/search.php?searchtype=articlename&searchkey=${key}`;
  ...
  getIntroduction() {
    ...
    return this.search().then((id) => {
      data.id = id;
      const url = `http://x23us.com/book/${id}`;
      return utils.gbkGet(url);
    }).then((res) => {
